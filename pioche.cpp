...
VuePioche::VuePioche(QWidget * parent): QWidget(parent) {
    ...
    for (size_t row = 0; row < p3->getMaxCartesRevelees(); row++) {
        std::cout << "Test \n";
        VueCarteJoaillerie * carte = new VueCarteJoaillerie(p3->cartes_dehors[row], test, nullptr);
        ...
        connect(vuecartes3[row], SIGNAL(clicked()), this, SLOT(carteClique(vuecartes3[row])));
    }
    ...
    for (size_t row = 0; row < p2->getMaxCartesRevelees(); row++) {
        ...
        connect(vuecartes2[row], SIGNAL(clicked()), this, SLOT(carteClique(vuecartes2[row])));
    }
    ...
    for (size_t row = 0; row < p1->getMaxCartesRevelees(); row++) {
        ...
        connect(vuecartes1[row], SIGNAL(clicked()), this, SLOT(carteClique(vuecartes1[row])));
    }
    ...
    QPushButton * dos3 = new QPushButton("Pioche 2");
...
int VuePioche::carteClique(VueCarteJoaillerie * c) {
    std::cout << "ID : " << c->getCarteJoaillerieId() << "\n";
    return c->getCarteJoaillerieId();
}